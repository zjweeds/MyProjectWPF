...
namespace Controllers.MyControls
{
    ...
    public partial class CComboBox : ComboBox
    {
        ...
        //定义一个方法，作为自定义控件CComboBox的鼠标按下事件的绑定方法
        private void CComboBox_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        public List<CComboBox> GetCComboBoxes(Control control)
        {
            List<CComboBox> ctxts = new List<CComboBox>();
            foreach (Control con in control.Controls)
            {
                if (con.GetType() == typeof(CComboBox))
                {
                    ctxts.Add((CComboBox)con);
                }
                if (con.GetType() == typeof(GroupBox))
                {
                    this.GetCComboBoxes(con);
                }
            }
            return ctxts;
        }
        ...
        //自定义控件的鼠标移动事件绑定方法
        private void CComboBox_MouseMove(object sender, MouseEventArgs e)
        {
            if (isMoving)
            {
                //根据鼠标移动动态地设置控件的位置
        ...
        private void CComboBox_MouseUp(object sender, MouseEventArgs e)
        {
            isMoving = false;
        }

        private void toolDeleteCComboBox_Click(object sender, EventArgs e)
        {
