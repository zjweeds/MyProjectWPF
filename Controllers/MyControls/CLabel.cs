...
namespace Controllers.MyControls
{
    ...
    public partial class CLabel : Label
    {
        ...
        //定义一个方法，作为自定义控件Clabel的鼠标按下事件的绑定方法
        private void Clabel_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        public List<CLabel> GetClabeles(Control control)
        {
        ...
        //自定义控件的鼠标移动事件绑定方法
        private void Clabel_MouseMove(object sender, MouseEventArgs e)
        {
            if (isMoving)
            {
                //根据鼠标移动动态地设置控件的位置
        ...
        private void toolDeleteClabel_Click(object sender, EventArgs e)
        {
            ...
            //            {
            //                if (MessageBox.Show("该输入框已生成单据数据，若继续执行将删除与之相关的数据，是否继续执行？", "软件提示", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.No)
            //                {
            //                    return;  //停止执行
            //                }
        ...
        {
            List<CLabel> ctxts = GetClabeles(this.FormParent);
